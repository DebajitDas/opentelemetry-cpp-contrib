...
public:
  ...
  MOCK_METHOD(void,
              AddAttribute,
              (const std::string &key, const appd::core::sdkwrapper::SpanAttributeValue &value),
              (override, noexcept));

  MOCK_METHOD(void,
              SetStatus,
              (const appd::core::sdkwrapper::StatusCode code, const std::string &desc),
              (override));

  MOCK_METHOD(appd::core::sdkwrapper::SpanKind, GetSpanKind, (), (override));
};

// TODO : General MOCK_METHOD command is giving some unexpected errors. Revisit
// later
class MockSdkWrapper : public appd::core::sdkwrapper::ISdkWrapper
{
public:
  void Init(std::shared_ptr<appd::core::TenantConfig>) override {}
  MOCK_METHOD1(PopulatePropagationHeaders,
               void(std::unordered_map<std::string, std::string> &carrier));
  MOCK_METHOD4(CreateSpan,
               std::shared_ptr<appd::core::sdkwrapper::IScopedSpan>(
                   const std::string &name,
