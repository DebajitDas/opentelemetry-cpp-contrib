...
TEST(ScopedSpan, ScopedSpan_Create)
{
  ...
  ScopedSpan testSpan(kTestSpanName, opentelemetry::trace::SpanKind::kInternal, attributes,
...
TEST(ScopedSpan, ScopedSpan_End)
{
  ...
  EXPECT_CALL(*testTracer, StartSpanInternal(_, _))
      .Times(1)
      .WillOnce(InvokeWithoutArgs(ActionGetSpans));
  ...
  EXPECT_CALL(mockSdkHelperFactory, GetTracer())
      ...
      .WillRepeatedly(InvokeWithoutArgs(ActionGetTracer));
  AgentLogger logger = getLogger("Test");
  ScopedSpan testScopedSpan(kTestSpanName, opentelemetry::trace::SpanKind::kInternal,
                            OtelKeyValueMap{}, &mockSdkHelperFactory, logger);
...
TEST(ScopedSpan, ScopedSpan_AddAttribute)
{
  ...
  EXPECT_CALL(*testSpan, SetAttribute(testkey, HasStringValue("TestValue"))).Times(1);
