...
TEST(ScopedSpan, ScopedSpan_Create) {
  ...
  ScopedSpan testSpan(kTestSpanName, opentelemetry::trace::SpanKind::kInternal,
                      attributes, &mockSdkHelperFactory, logger);
}
...
TEST(ScopedSpan, ScopedSpan_AddAttribute) {
  ...
  EXPECT_CALL(*testSpan, SetAttribute(testkey, HasStringValue("TestValue")))
      .Times(1);
