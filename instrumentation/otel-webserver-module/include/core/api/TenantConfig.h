...
public:
  ...
  // This function is used to copy all the attribute from the initial core init
  // config to the user added tenants. Since we only allow the user to specify
  // service namespace/name/id, we must get all the other attributes from the
  // core initial config
...
private:
  ...
  // Span Limits(AttributeCountLimit, EventCountLimit, LinkCountLimit,
  // AttributePerEventCountLimit, AttributePerLinkCountLimit
  //: configuration options have not been added for these; as of now they will
