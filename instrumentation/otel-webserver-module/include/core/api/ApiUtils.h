...
std::string& result); virtual APPD_SDK_STATUS_CODE ReadOptional(const
...
protected:
  APPD_SDK_STATUS_CODE ReadSettingsFromReader(IEnvReader &reader,
                                              TenantConfig &,
                                              SpanNamer &) override;
  /*APPD_SDK_STATUS_CODE ReadMandatoryFromReader(
      IEnvReader& reader, const std::string& varName, unsigned short& result)
     override;*/
  APPD_SDK_STATUS_CODE ReadOptionalFromReader(IEnvReader &reader,
