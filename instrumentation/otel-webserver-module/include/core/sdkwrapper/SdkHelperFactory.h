...
private:
  ...
  OtelSpanProcessor GetSpanProcessor(std::shared_ptr<TenantConfig> config,
                                     OtelSpanExporter exporter);

  OtelSampler GetSampler(std::shared_ptr<TenantConfig> config);

  // There is one-to-one mapping of WebserverContext(Virtual Host) to
  // TracerProvider. Therefore, tracer provided can't be stored as global
  // variable and needs to be accessed per WebserverContext.
