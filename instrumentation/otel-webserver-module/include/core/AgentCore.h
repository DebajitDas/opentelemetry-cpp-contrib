...
protected:
  ...
  // Todo: Add availble resources that need to be shared among all the
  // worker-threads
};
...
public:
  ...
  IRequestProcessingEngine *getRequestProcessor(std::string &name) override;

  void addContext(const std::string &contextName,
                  std::shared_ptr<appd::core::TenantConfig> newConfig) override;

  std::shared_ptr<IContext> getWebServerContext(std::string &name) override;
