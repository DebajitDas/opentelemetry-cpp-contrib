...
public:
  ...
  // int appd_hook_generate_log_id(const conn_rec *c, const request_rec *r,
  // const char**);

  /*
      function prototypes for the apache module hook callbacks according to
     different stages
  */
  ...
  /*
      ...
      These hooks are stage specific and each hook corresponds to a particular
     module. The number of hooks are proportional to the number of modules
     instrumented for a stage. See "AppDynamicsReportAllInstrumentedModules"
     flag for more details.
      TODO: Decide among the following stages at what all we need the modules to
     be instrumented, and define the hooks handlers for the same to start an
     interaction before module callback.
  */
