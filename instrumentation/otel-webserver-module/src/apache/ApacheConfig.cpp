...
const char *ApacheConfigHandlers::helperChar(cmd_parms *cmd,
                                             appd_cfg *cfg,
                                             const char *arg,
                                             const char *&var,
                                             int &initialized,
...
const char *ApacheConfigHandlers::helperInt(cmd_parms *cmd,
                                            appd_cfg *cfg,
                                            const char *arg,
                                            int &var,
                                            int &initialized,
...
void ApacheConfigHandlers::insertWebserverContext(cmd_parms *cmd, const appd_cfg *cfg)
{
  ...
  {
    ApacheTracing::writeError(cmd->server, "Config", "Cannot create context name (%s,%s,%s)",
                              cfg->serviceNamespace, cfg->serviceName, cfg->serviceInstanceId);
    return;
  }

  ApacheTracing::writeTrace(cmd->server, "Config", "Context %s:%s,%s,%s", contextName.c_str(),
                            cfg->serviceNamespace, cfg->serviceName, cfg->serviceInstanceId);
  ...
  if (it != m_webServerContexts.end())  // context already exists
  {
        ...
        it->second->m_serviceInstanceId != cfg->serviceInstanceId)
    {
      ...
      ApacheTracing::writeError(cmd->server, "Config", "Context %s already exists",
                                contextName.c_str());
    }
...
const char *ApacheConfigHandlers::appd_set_enabled(cmd_parms *cmd, void *conf, const char *arg)
{
  ...
  return helperInt(cmd, cfg, arg, cfg->appdEnabled, cfg->appdEnabled_initialized,
                   "appd_set_enabled");
}

//  char *otelExporterEndpoint;
//  int otelExporterEndpoint_initialized;
const char *ApacheConfigHandlers::appd_set_otelExporterEndpoint(cmd_parms *cmd,
                                                                void *conf,
                                                                const char *arg)
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperChar(cmd, cfg, arg, cfg->otelExporterEndpoint, cfg->otelExporterEndpoint_initialized,
...
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperInt(cmd, cfg, arg, cfg->otelSslEnabled, cfg->otelSslEnabled_initialized,
                   "appd_set_otelSslEnabled");
}

//  char *otelSslCertificatePath;
//  int otelSslCertificatePath_initialized;
const char *ApacheConfigHandlers::appd_set_otelSslCertificatePath(cmd_parms *cmd,
                                                                  void *conf,
                                                                  const char *arg)
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperChar(cmd, cfg, arg, cfg->otelSslCertificatePath,
                    cfg->otelSslCertificatePath_initialized, "appd_set_otelSslCertificatePath");
}
...
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperChar(cmd, cfg, arg, cfg->otelExporterType, cfg->otelExporterType_initialized,
...
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperChar(cmd, cfg, arg, cfg->otelProcessorType, cfg->otelProcessorType_initialized,
...
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperChar(cmd, cfg, arg, cfg->otelSamplerType, cfg->otelSamplerType_initialized,
                                                            ...
                                                            const char *arg)
{
  ...
  const char *rc = helperChar(cmd, cfg, arg, cfg->serviceNamespace,
                              cfg->serviceNamespace_initialized, "appd_set_serviceNamespace");
...
{
  appd_cfg *cfg  = (appd_cfg *)conf;
  const char *rc = helperChar(cmd, cfg, arg, cfg->serviceName, cfg->serviceName_initialized,
                                                             ...
                                                             const char *arg)
{
  ...
  const char *rc = helperChar(cmd, cfg, arg, cfg->serviceInstanceId,
                              cfg->serviceInstanceId_initialized, "appd_set_serviceInstanceId");
  if (cfg->serviceNamespace_initialized && cfg->serviceName_initialized &&
      cfg->serviceInstanceId_initialized)
  {
    // There are separate Apache directives for setting serviceNamespace,
...
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperChar(cmd, cfg, arg, cfg->otelMaxQueueSize, cfg->otelMaxQueueSize_initialized,
...
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperChar(cmd, cfg, arg, cfg->otelScheduledDelay, cfg->otelScheduledDelay_initialized,
...
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperChar(cmd, cfg, arg, cfg->otelExportTimeout, cfg->otelExportTimeout_initialized,
                    "appd_set_otelExportTimeout");
}

//  char *otelMaxExportBatchSize;
//  int otelMaxExportBatchSize_initialized;
const char *ApacheConfigHandlers::appd_set_otelMaxExportBatchSize(cmd_parms *cmd,
                                                                  void *conf,
                                                                  const char *arg)
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperChar(cmd, cfg, arg, cfg->otelMaxExportBatchSize,
                    cfg->otelMaxExportBatchSize_initialized, "appd_set_otelMaxExportBatchSize");
}
...
{
  appd_cfg *cfg = (appd_cfg *)conf;
  return helperInt(cmd, cfg, arg, cfg->resolveBackends, cfg->resolveBackends_initialized,
...
const char *ApacheConfigHandlers::appd_set_reportAllInstrumentedModules(cmd_parms *cmd,
                                                                        void *conf,
                                                                        const char *arg)
{
  appd_cfg *cfg = (appd_cfg *)conf;
  helperInt(cmd, cfg, arg, cfg->reportAllInstrumentedModules,
            cfg->reportAllInstrumentedModules_initialized, "appd_set_reportAllInstrumentedModules");
...
const char *ApacheConfigHandlers::appd_set_delimiter(cmd_parms *cmd, void *conf, const char *arg)
{
  ...
  if (cfg->delimiter_initialized && cfg->segment_initialized && cfg->matchFilter_initialized &&
      cfg->matchPattern_initialized)
  {
...
{
  appd_cfg *cfg = (appd_cfg *)conf;
  helperChar(cmd, cfg, arg, cfg->segmentParameter, cfg->segmentParameter_initialized,
             "appd_set_segmentParameter");
  if (cfg->segmentType_initialized && cfg->segmentParameter_initialized)
  {}
  return NULL;
}
...
const char *ApacheConfigHandlers::appd_add_webserver_context(cmd_parms *cmd,
                                                             void *conf,
...
void *ApacheConfigHandlers::appd_create_dir_config(apr_pool_t *p, char *dirspec)
{
  ...
  ApacheTracing::writeTrace(NULL, __func__, "(p == %p, dirspec == %s)", static_cast<void *>(p),
                            dname);
                                                  ...
                                                  void *newloc_conf)
{
  ...
  appd_cfg *merged_config =
      (appd_cfg *)appd_create_dir_config(p, (char *)"Merged Directory Configuration");
  ...
  // otelSslEnabled
  merged_config->otelSslEnabled =
      nconf->otelSslEnabled_initialized ? nconf->otelSslEnabled : pconf->otelSslEnabled;
  ...
  merged_config->serviceName = nconf->serviceName_initialized ? apr_pstrdup(p, nconf->serviceName)
  ...
  merged_config->resolveBackends =
      nconf->resolveBackends_initialized ? nconf->resolveBackends : pconf->resolveBackends;
  merged_config->resolveBackends_initialized = 1;
  merged_config->traceAsError =
      nconf->traceAsError_initialized ? nconf->traceAsError : pconf->traceAsError;
  ...
  merged_config->cookiePattern =
      nconf->cookiePattern_initialized ? nconf->cookiePattern : pconf->cookiePattern;
  ...
  merged_config->matchPattern =
      nconf->matchPattern_initialized ? nconf->matchPattern : pconf->matchPattern;
  ...
  merged_config->segmentParameter =
      nconf->segmentParameter_initialized ? nconf->segmentParameter : pconf->segmentParameter;
  merged_config->segmentParameter_initialized = 1;

  ApacheTracing::writeTrace(NULL, __func__, "(p == %p, parent_conf == %p, newloc_conf == %p)",
...
void ApacheConfigHandlers::traceConfig(const request_rec *r, const appd_cfg *cfg)
{
  ApacheTracing::writeTrace(
      ...
      "(SegmentParameter=\"%s\")"
      "}",
      cfg->appdEnabled, cfg->otelExporterEndpoint, cfg->otelSslEnabled, cfg->otelSslCertificatePath,
      cfg->otelExporterType, cfg->otelProcessorType, cfg->otelSamplerType, cfg->serviceNamespace,
      cfg->serviceName, cfg->serviceInstanceId, cfg->otelMaxQueueSize, cfg->otelScheduledDelay,
      cfg->otelExportTimeout, cfg->otelMaxExportBatchSize, cfg->resolveBackends, cfg->traceAsError,
      cfg->reportAllInstrumentedModules, cfg->maskCookie, cfg->maskSmUser, cfg->segmentType,
