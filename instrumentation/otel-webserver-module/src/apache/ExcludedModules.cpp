...
const std::unordered_set<std::string> ExcludedModules::excludedAlways{
    "mod_apache_otel.cpp"  // filter out our own mod! else insertHook will
                           // infinite loop!
};
...
const std::unordered_set<std::string> ExcludedModules::excluded24{"core.c",
                                                                  "http_core.c",
                                                                  "mod_access_compat.c",
                                                                  "mod_actions.c",
                                                                  "mod_alias.c",
...
const std::unordered_set<std::string> ExcludedModules::excluded22{"core.c",
                                                                  "http_core.c",
                                                                  "mod_actions.c",
                                                                  "mod_alias.c",
                                                                  "mod_auth_basic.c",
...
// Add modules that are specified by the user and  modules NOT in the exclude
// lists.
void ExcludedModules::findHookPoints(std::vector<HookInfo> &hooks_found,
                                     hook_get_t getHooks,
  ...
  if (hooks)
  {
    ...
    for (int ix = 0; ix < hooks->nelts; ix++)
    {
      ...
      if (excludedAlways.find(elts[ix].szName) == excludedAlways.end())
      {
        ...
        {  // TODO: find a way to determine if we are
           // in 2.2 or 2.4 and only use one of these.
          hooks_found.push_back((HookInfo(stage, elts[ix].szName, elts[ix].nOrder)));
        }
      }
    }
...
{
  /*
      We'll ask the user to provide us with specific modules which need to be
     monitored either through configuration file or through the UI. Push the
     modules into a list/map.
  */
}
