...
void ApacheHooks::registerHooks(apr_pool_t *p) {
  ...
  ap_hook_log_transaction(ApacheHooks::appd_hook_log_transaction_end, NULL,
                          NULL, APR_HOOK_REALLY_LAST + 2);
  ...
  ap_register_output_filter(APPD_OUTPUT_FILTER_NAME,
                            ApacheHooks::appd_output_filter, NULL,
                            AP_FTYPE_CONTENT_SET);
  ap_hook_insert_filter(ApacheHooks::appd_hook_interaction_end_insert_filter,
                        NULL, NULL, APR_HOOK_REALLY_FIRST - 2);
  ...
  // stage order.
  ap_hook_quick_handler(ApacheHooks::appd_hook_interaction_end_quick_handler,
                        NULL, NULL, APR_HOOK_REALLY_FIRST - 2);
  ...
  ap_hook_type_checker(ApacheHooks::appd_hook_interaction_end, NULL, NULL,
                       APR_HOOK_REALLY_FIRST - 2);
  ap_hook_fixups(ApacheHooks::appd_hook_interaction_end, NULL, NULL,
                 APR_HOOK_REALLY_FIRST - 2);
  ap_hook_handler(ApacheHooks::appd_hook_interaction_end, NULL, NULL,
                  APR_HOOK_REALLY_FIRST - 2);
  ap_hook_log_transaction(
      ApacheHooks::appd_hook_interaction_end_log_transaction, NULL, NULL,
      APR_HOOK_REALLY_FIRST - 2);
  ...
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_header_parser, ap_hook_header_parser,
  ...
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_quick_handler, ap_hook_quick_handler,
      ApacheHooksForStage::appd_quick_handler_hooks,
      ApacheHooksForStage::appd_quick_handler_indexes,
      ApacheHooks::appd_hook_interaction_end_quick_handler, "quick_handler");
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_access_checker, ap_hook_access_checker,
      ApacheHooksForStage::appd_access_checker_hooks,
      ApacheHooksForStage::appd_access_checker_indexes,
      ApacheHooks::appd_hook_interaction_end, "access_checker");
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_check_user_id, ap_hook_check_user_id,
  ...
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_auth_checker, ap_hook_auth_checker,
  ...
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_type_checker, ap_hook_type_checker,
  ...
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_fixups, ap_hook_fixups,
      ApacheHooksForStage::appd_fixups_hooks,
      ApacheHooksForStage::appd_fixups_indexes,
      ApacheHooks::appd_hook_interaction_end, "fixups");
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_insert_filter, ap_hook_insert_filter,
      ApacheHooksForStage::appd_insert_filter_hooks,
      ApacheHooksForStage::appd_insert_filter_indexes,
      ApacheHooks::appd_hook_interaction_end_insert_filter, "insert_filter");
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_handler, ap_hook_handler,
      ApacheHooksForStage::appd_handler_hooks,
      ApacheHooksForStage::appd_handler_indexes,
      ApacheHooks::appd_hook_interaction_end_handler, "handler");
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_log_transaction, ap_hook_log_transaction,
      ApacheHooksForStage::appd_log_transaction_hooks,
      ApacheHooksForStage::appd_log_transaction_indexes,
      ApacheHooks::appd_hook_interaction_end, "log_transaction");
}

apr_status_t ApacheHooks::appd_output_filter(ap_filter_t *f,
...
// end the interaction
void ApacheHooks::appd_stopInteraction(request_rec *r, bool isAlwaysRunStage,
                                       bool ignoreBackend) {
  if ((!isAlwaysRunStage && !m_reportAllStages) || !r || !r->notes ||
      !ap_is_initial_req(r)) {
    return;
  }
  ...
  if (reqHandlePtr) {
    ...
    std::unique_ptr<appd::core::EndInteractionPayload> payload(
        new appd::core::EndInteractionPayload(backendName, backendType,
                                              errorCode, oss.str()));
    APPD_SDK_STATUS_CODE res =
        wsAgent.endInteraction(reqHandlePtr, ignoreBackend, payload.get());
...
APPD_SDK_STATUS_CODE ApacheHooks::appd_startInteraction(
    request_rec *r, HookContainer::appd_endpoint_indexes endpointIndex,
    bool isAlwaysRunStage, bool ignoreBackend) {
  /*
  ...
  */
  ...
  const std::string &stage =
      HookContainer::getInstance().getStage(endpointIndex);
  const std::string &module =
      HookContainer::getInstance().getModule(endpointIndex);
  ...
  if (reqHandlePtr) {
    ...
    res = wsAgent.startInteraction(reqHandlePtr, payload.get(),
                                   propagationHeaders);
...
void ApacheHooks::appd_payload_decorator(
    request_rec *request,
    std::unordered_map<std::string, std::string> propagationHeaders) {
  for (auto itr = propagationHeaders.begin(); itr != propagationHeaders.end();
       itr++) {
...
bool ApacheHooks::initialize_appdynamics(const request_rec *r) {
  ...
  apr_pool_userdata_get(&config_data, APPD_CONFIG_KEY,
                        r->server->process->pool);

  if (config_data != NULL) {
    ApacheTracing::writeTrace(r->server, __func__,
                              "config retrieved from process memory pool");
    return true;
  }
  ...
  ApacheTracing::writeTrace(r->server, __func__,
                            "no config stored in the process memory pool");
  ...
  if (our_config->getAppdEnabled() == 1) {
    ...
    APPD_SDK_ENV_RECORD *env_config = (APPD_SDK_ENV_RECORD *)apr_pcalloc(
        r->pool, 16 * sizeof(APPD_SDK_ENV_RECORD));
    int ix = 0;
    ...
    if (APPD_ISSUCCESS(res)) {
      ...
      apr_pool_userdata_set((const void *)process_cfg, APPD_CONFIG_KEY,
                            apr_pool_cleanup_null, r->server->process->pool);

      return true;
    } else {
      ApacheTracing::writeError(r->server, __func__,
                                "SDK Init failed, result code is: %d", res);
      return false;
    }
  } else {
...
void fillRequestPayload(request_rec *request,
                        appd::core::RequestPayload *payload) {
  const char *val;

  // httpHeaders
  for (auto itr = ApacheHooks::httpHeaders.begin();
       itr != ApacheHooks::httpHeaders.end(); ++itr) {
    const char *hdr = *itr;
  ...
  if (!request->method || !request->args ||
      apr_strnatcmp(request->method, "POST")) {
    val = " ";
  } else
    val = request->args;
  ...
  // http_get_parameter
  if (!request->method || !request->args ||
      apr_strnatcmp(request->method, "GET")) {
    val = " ";
  } else
    val = request->args;
...
int ApacheHooks::appd_hook_header_parser_begin(request_rec *r) {
  if (!ap_is_initial_req(r)) {
    ...
    ApacheTracing::writeTrace(r->server, __func__,
                              "appdynamics did not get initialized");
    return DECLINED;
  }
  ...
  if (APPD_ISSUCCESS(res)) {
    ...
    if (reqHandleValue) {
      ...
      apr_table_setn(r->notes, APPD_REQ_HANDLE_KEY,
                     (const char *)reqHandleValue);
    }
    ...
    ApacheTracing::writeTrace(r->server, __func__,
                              "request begin error, result code: %d", res);
  }
...
int ApacheHooks::appd_hook_log_transaction_end(request_rec *r) {
  ...
  */
  ...
  appd_cfg *cfg = NULL;
  apr_pool_userdata_get((void **)&cfg, APPD_CONFIG_KEY,
                        r->server->process->pool);
  ...
  if (APPD_ISSUCCESS(res)) {
    ApacheTracing::writeTrace(r->server, __func__,
                              "request end successful (HTTP status=%d)",
                              r->status);
  } else {
    ApacheTracing::writeTrace(r->server, __func__,
                              "request end result code: %d", res);
  }
...
apr_status_t ApacheHooks::appd_child_exit(void *data) {
  ...
  if (our_config) {
    ApacheTracing::writeError(s, __func__, "terminating agent (sname=%s)",
                              sname);
    wsAgent.term();
  }
...
int ApacheHooks::appd_hook_interaction_end_quick_handler(request_rec *r,
                                                         int i) {
  appd_stopInteraction(r);
  return DECLINED;
}
...
void ApacheHooksForStage::insertHooksForStage(
  ...
  for (auto it = hooksNeeded.begin(); it != hooksNeeded.end(); it++) {
    if (hookCount < beginHandlers.size()) {
      HookContainer::getInstance().addHook(indexes[hookCount], stage,
                                           it->module, it->order);
      ...
      ApacheTracing::writeTrace(
          NULL, __func__, "Instrumentation Hooks added. Stage: %s Module: %s",
...
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_header_parser_hooks = {
        appd_hook_header_parser1, appd_hook_header_parser2,
        appd_hook_header_parser3, appd_hook_header_parser4,
...
const std::vector<ApacheHooksForStage::quickHandlerHooks>
    ApacheHooksForStage::appd_quick_handler_hooks = {
        appd_hook_quick_handler1, appd_hook_quick_handler2,
        appd_hook_quick_handler3, appd_hook_quick_handler4,
...
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_access_checker_hooks = {
        appd_hook_access_checker1, appd_hook_access_checker2,
        appd_hook_access_checker3, appd_hook_access_checker4,
...
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_check_user_id_hooks = {
        ...
        appd_hook_check_user_id3, appd_hook_check_user_id4,
...
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_auth_checker_hooks = {
        appd_hook_auth_checker1, appd_hook_auth_checker2,
        appd_hook_auth_checker3, appd_hook_auth_checker4,
...
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_type_checker_hooks = {
        appd_hook_type_checker1, appd_hook_type_checker2,
        appd_hook_type_checker3, appd_hook_type_checker4,
...
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_fixups_hooks = {
        appd_hook_fixups1, appd_hook_fixups2, appd_hook_fixups3,
        appd_hook_fixups4, appd_hook_fixups5};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_fixups_indexes = {
...
const std::vector<ApacheHooksForStage::filterHooks>
    ApacheHooksForStage::appd_insert_filter_hooks = {
        appd_hook_insert_filter1, appd_hook_insert_filter2,
        appd_hook_insert_filter3, appd_hook_insert_filter4,
...
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_handler_hooks = {
        appd_hook_handler1,  appd_hook_handler2,  appd_hook_handler3,
        appd_hook_handler4,  appd_hook_handler5,  appd_hook_handler6,
        appd_hook_handler7,  appd_hook_handler8,  appd_hook_handler9,
        appd_hook_handler10, appd_hook_handler11, appd_hook_handler12,
        appd_hook_handler13, appd_hook_handler14, appd_hook_handler15,
        appd_hook_handler16, appd_hook_handler17, appd_hook_handler18,
        appd_hook_handler19, appd_hook_handler20};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_handler_indexes = {
...
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_log_transaction_hooks = {
        appd_hook_log_transaction1, appd_hook_log_transaction2,
        appd_hook_log_transaction3, appd_hook_log_transaction4,
