...
const std::initializer_list<const char *> ApacheHooks::httpHeaders = {"baggage", "traceparent",
                                                                      "tracestate"};
...
void ApacheHooks::registerHooks(apr_pool_t *p)
{
  ...
  //------------------------------------------------------------------------------
  // The -2 and +2 below is a trick to ensure that our hooks are before and
  // after the majority of the modules within a stage. This is not in the
  // official apache documentation. There is no guarantee that we will
  // instrument a module if it also does this +2/-2 trick.
  //------------------------------------------------------------------------------

  // request_begin and request_end hooks
  ap_hook_header_parser(ApacheHooks::appd_hook_header_parser_begin, NULL, NULL,
                        APR_HOOK_REALLY_FIRST - 2);
  ap_hook_log_transaction(ApacheHooks::appd_hook_log_transaction_end, NULL, NULL,
                          APR_HOOK_REALLY_LAST + 2);

  // Register the output filter after the response resource is generated
  // (AP_FTYPE_CONTENT_SET). The filter gets added into the chain in
  // appd_hook_interaction_end_insert_filter via ap_hook_insert_filter.
  ap_register_output_filter(APPD_OUTPUT_FILTER_NAME, ApacheHooks::appd_output_filter, NULL,
                            AP_FTYPE_CONTENT_SET);
  ap_hook_insert_filter(ApacheHooks::appd_hook_interaction_end_insert_filter, NULL, NULL,
                        APR_HOOK_REALLY_FIRST - 2);

  // TODO: For the time being put in a hook at the very beginning of every stage
  // we are instrumenting. This is a catch all to stop a interaction. We can be
  // smarter and only put it in when we know that there is a module we care
  // about for a previous stage. This means we need to know the deterministic
  // stage order.
  ap_hook_quick_handler(ApacheHooks::appd_hook_interaction_end_quick_handler, NULL, NULL,
                        APR_HOOK_REALLY_FIRST - 2);
  ap_hook_access_checker(ApacheHooks::appd_hook_interaction_end, NULL, NULL,
                         APR_HOOK_REALLY_FIRST - 2);
  ap_hook_check_user_id(ApacheHooks::appd_hook_interaction_end, NULL, NULL,
                        APR_HOOK_REALLY_FIRST - 2);
  ap_hook_auth_checker(ApacheHooks::appd_hook_interaction_end, NULL, NULL,
                       APR_HOOK_REALLY_FIRST - 2);
  ap_hook_type_checker(ApacheHooks::appd_hook_interaction_end, NULL, NULL,
                       APR_HOOK_REALLY_FIRST - 2);
  ...
  ap_hook_log_transaction(ApacheHooks::appd_hook_interaction_end_log_transaction, NULL, NULL,
                          APR_HOOK_REALLY_FIRST - 2);

  // Stage Hooks
  // TODO: Decide among the following stages at what all we need the modules to
  // be instrumented,
  //       and define the hooks handlers for the same to start an interaction
  //       before module callback.
  ApacheHooksForStage::insertHooksForStage(p, ap_hook_get_header_parser, ap_hook_header_parser,
                                           ApacheHooksForStage::appd_header_parser_hooks,
                                           ApacheHooksForStage::appd_header_parser_indexes,
                                           ApacheHooks::appd_hook_interaction_end, "header_parser");
  ApacheHooksForStage::insertHooksForStage(p, ap_hook_get_quick_handler, ap_hook_quick_handler,
                                           ...
                                           "quick_handler");
  ApacheHooksForStage::insertHooksForStage(p, ap_hook_get_access_checker, ap_hook_access_checker,
                                           ...
                                           "access_checker");
  ApacheHooksForStage::insertHooksForStage(p, ap_hook_get_check_user_id, ap_hook_check_user_id,
                                           ApacheHooksForStage::appd_check_user_id_hooks,
                                           ApacheHooksForStage::appd_check_user_id_indexes,
                                           ApacheHooks::appd_hook_interaction_end, "check_user_id");
  ApacheHooksForStage::insertHooksForStage(p, ap_hook_get_auth_checker, ap_hook_auth_checker,
                                           ApacheHooksForStage::appd_auth_checker_hooks,
                                           ApacheHooksForStage::appd_auth_checker_indexes,
                                           ApacheHooks::appd_hook_interaction_end, "auth_checker");
  ApacheHooksForStage::insertHooksForStage(p, ap_hook_get_type_checker, ap_hook_type_checker,
                                           ApacheHooksForStage::appd_type_checker_hooks,
                                           ApacheHooksForStage::appd_type_checker_indexes,
                                           ApacheHooks::appd_hook_interaction_end, "type_checker");
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_fixups, ap_hook_fixups, ApacheHooksForStage::appd_fixups_hooks,
      ApacheHooksForStage::appd_fixups_indexes, ApacheHooks::appd_hook_interaction_end, "fixups");
  ApacheHooksForStage::insertHooksForStage(p, ap_hook_get_insert_filter, ap_hook_insert_filter,
                                           ...
                                           "insert_filter");
  ApacheHooksForStage::insertHooksForStage(
      p, ap_hook_get_handler, ap_hook_handler, ApacheHooksForStage::appd_handler_hooks,
      ApacheHooksForStage::appd_handler_indexes, ApacheHooks::appd_hook_interaction_end_handler,
      "handler");
  ApacheHooksForStage::insertHooksForStage(p, ap_hook_get_log_transaction, ap_hook_log_transaction,
...
apr_status_t ApacheHooks::appd_output_filter(ap_filter_t *f, apr_bucket_brigade *bb)
{
  if (APR_BRIGADE_EMPTY(bb))
  {
    ...
    // chain." https://httpd.apache.org/docs/trunk/developer/output-filters.html
    return APR_SUCCESS;
  }
...
void ApacheHooks::appd_stopInteraction(request_rec *r, bool isAlwaysRunStage, bool ignoreBackend)
{
  ...
  APPD_SDK_HANDLE_REQ reqHandlePtr =
      (APPD_SDK_HANDLE_REQ)apr_table_get(r->notes, APPD_REQ_HANDLE_KEY);
  if (reqHandlePtr)
  {
    ...
    std::unique_ptr<appd::core::EndInteractionPayload> payload(
        new appd::core::EndInteractionPayload(backendName, backendType, errorCode, oss.str()));
...
{
  /*
      Retrieve the stage and module name from endpointIndex and start an
     interaction to module.
  */
  ...
  if (reqHandlePtr)
  {
    // In case a previous "end hook" is never called. End current interaction
    ...
    std::unique_ptr<appd::core::InteractionPayload> payload(
        new appd::core::InteractionPayload(module, stage, resolveBackends));
    ...
    if (APPD_ISSUCCESS(res))
    {
      ...
      else
      {
        ApacheTracing::writeTrace(r->server, module.c_str(),
                                  "propagationHeaders were not filled, cannot add correlation "
    ...
    {
      ApacheTracing::writeTrace(r->server, module.c_str(),
                                "Error: interaction begin result code: %d", res);
    }
  }
  return res;
}
...
//-----------------------------------------------------------------------------------------
void ApacheHooks::appd_payload_decorator(
    request_rec *request,
    std::unordered_map<std::string, std::string> propagationHeaders)
{
  for (auto itr = propagationHeaders.begin(); itr != propagationHeaders.end(); itr++)
  {
    ...
    ApacheTracing::writeTrace(request->server, __func__,
                              "correlation information : \"%s\" : \"%s\"", itr->first.c_str(),
                              itr->second.c_str());
  }
}
...
{
  // check to see if we have already been initialized by getting user data from
  // the process pool
  ...
  if (our_config->getAppdEnabled() == 1)
  {
    ...
    // !!!
    // Remember to update the apr_pcalloc call size if we add another parameter
    // to the input array!
    // !!!

    for (auto it = ApacheConfigHandlers::m_webServerContexts.begin();
         it != ApacheConfigHandlers::m_webServerContexts.end(); ++it)
    {
      ...
      ApacheTracing::writeTrace(r->server, __func__,
                                "Adding Context: %s ServiceNamespace: %s "
                                "ServiceName: %s ServiceInstanceId: %s",
                                it->first.c_str(), it->second->m_serviceNamespace.c_str(),
    ...
    if (APPD_ISSUCCESS(res))
    {
      ...
      // Store the config that was selected for all other methods to be able to
      // use
      appd_cfg *process_cfg = ApacheConfigHandlers::getProcessConfig(r);
      apr_pool_userdata_set((const void *)process_cfg, APPD_CONFIG_KEY, apr_pool_cleanup_null,
...
// post_read_request is too early in the cycle to get the config parameters for
...
int ApacheHooks::appd_hook_header_parser_begin(request_rec *r)
{
  ...
  ApacheTracing::writeTrace(r->server, __func__, "%s",
                            (wscontext ? wscontext : "using default context"));
  ...
  if (APPD_ISSUCCESS(res))
  {
    ...
    APPD_SDK_HANDLE_REQ reqHandleValue =
        (APPD_SDK_HANDLE_REQ)apr_palloc(r->pool, sizeof(APPD_SDK_HANDLE_REQ));
  ...
  else if (res == APPD_STATUS(cfg_channel_uninitialized) ||
           res == APPD_STATUS(bt_detection_disabled))
  {
    ApacheTracing::writeTrace(r->server, __func__,
                              "request begin detection disabled, result code: %d", res);
...
int ApacheHooks::appd_hook_log_transaction_end(request_rec *r)
{
  ...
  */
  ...
  // TODO - Determine whether there were any errors from any other part of this
  // request processing (even prior to our handler)
  ...
  if (APPD_ISSUCCESS(res))
  {
    ApacheTracing::writeTrace(r->server, __func__, "request end successful (HTTP status=%d)",
                              r->status);
  }
  else
  {
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HEADER_PARSER1);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_header_parser2(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HEADER_PARSER2);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_header_parser3(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HEADER_PARSER3);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_header_parser4(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HEADER_PARSER4);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_header_parser5(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HEADER_PARSER5);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_quick_handler1(request_rec *r, int i)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_QUICK_HANDLER1);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_quick_handler2(request_rec *r, int i)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_QUICK_HANDLER2);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_quick_handler3(request_rec *r, int i)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_QUICK_HANDLER3);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_quick_handler4(request_rec *r, int i)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_QUICK_HANDLER4);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_quick_handler5(request_rec *r, int i)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_QUICK_HANDLER5);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_access_checker1(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_ACCESS_CHECKER1);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_access_checker2(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_ACCESS_CHECKER2);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_access_checker3(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_ACCESS_CHECKER3);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_access_checker4(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_ACCESS_CHECKER4);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_access_checker5(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_ACCESS_CHECKER5);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_check_user_id1(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_CHECK_USER_ID1);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_check_user_id2(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_CHECK_USER_ID2);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_check_user_id3(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_CHECK_USER_ID3);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_check_user_id4(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_CHECK_USER_ID4);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_check_user_id5(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_CHECK_USER_ID5);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_auth_checker1(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_AUTH_CHECKER1);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_auth_checker2(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_AUTH_CHECKER2);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_auth_checker3(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_AUTH_CHECKER3);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_auth_checker4(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_AUTH_CHECKER4);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_auth_checker5(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_AUTH_CHECKER5);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_type_checker1(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_TYPE_CHECKER1);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_type_checker2(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_TYPE_CHECKER2);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_type_checker3(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_TYPE_CHECKER3);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_type_checker4(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_TYPE_CHECKER4);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_type_checker5(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_TYPE_CHECKER5);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_fixups1(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_FIXUPS1);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_fixups2(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_FIXUPS2);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_fixups3(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_FIXUPS3);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_fixups4(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_FIXUPS4);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_fixups5(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_FIXUPS5);
  return DECLINED;
}

void ApacheHooksForStage::appd_hook_insert_filter1(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_INSERT_FILTER1);
}

void ApacheHooksForStage::appd_hook_insert_filter2(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_INSERT_FILTER2);
}

void ApacheHooksForStage::appd_hook_insert_filter3(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_INSERT_FILTER3);
}

void ApacheHooksForStage::appd_hook_insert_filter4(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_INSERT_FILTER4);
}

void ApacheHooksForStage::appd_hook_insert_filter5(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_INSERT_FILTER5);
}

// The HANDLER stage does not always run all the hooks.  After a hook returns
// "OK", the stage will end and the remaining hooks are ignored. We use this
// behavior to ignore any backend calls that do not go out to another tier.
// Thus, if we hit any of the backend_end hook in HANDLER, we ignore that
// backend call in the SDK.
//
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER1,
                                     true,   // always run
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER2,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER3,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER4,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER5,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER6,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER7,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER8,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER9,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER10,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER11,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER12,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER13,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER14,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER15,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER16,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER17,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER18,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER19,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_HANDLER20,
                                     true,   // always run
                                     true);  // ignore backend if one exists
  return DECLINED;
}
...
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_LOG_TRANSACTION1);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_log_transaction2(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_LOG_TRANSACTION2);
  return DECLINED;
}

int ApacheHooksForStage::appd_hook_log_transaction3(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_LOG_TRANSACTION3);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_log_transaction4(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_LOG_TRANSACTION4);
  return DECLINED;
}
int ApacheHooksForStage::appd_hook_log_transaction5(request_rec *r)
{
  ApacheHooks::appd_startInteraction(r, HookContainer::APPD_ENDPOINT_LOG_TRANSACTION5);
  return DECLINED;
}
...
// The HANDLER stage does not always run all the hooks.  After a hook returns
// "OK", the stage will end and the remaining hooks are ignored. We use this
// behavior to ignore any endpoint calls that do not go out to another tier.
// Thus, if we hit any of the interaction_end hook in HANDLER, we ignore that
// endpoint call in the SDK.
//
// ASSUMPTION: if a module returns DECLINED, it does not go out to another tier.
int ApacheHooks::appd_hook_interaction_end_handler(request_rec *r)
{
  appd_stopInteraction(r, true,
                       true);  // always run and ignore backend if triggered
  return DECLINED;
}
    ...
    const std::string &stage)
{
  ...
  for (auto it = hooksNeeded.begin(); it != hooksNeeded.end(); it++)
  {
    if (hookCount < beginHandlers.size())
    {
      ...
      const char *const *moduleList =
          HookContainer::getInstance().getModuleList(indexes[hookCount]);

      setHook(beginHandlers[hookCount], NULL, moduleList, it->order);
      setHook(endHandler, moduleList, NULL, it->order);
      hookCount++;

      ApacheTracing::writeTrace(NULL, __func__, "Instrumentation Hooks added. Stage: %s Module: %s",
                                stage.c_str(), it->module.c_str());
    }
    else
    {
      ApacheTracing::writeTrace(NULL, __func__,
                                "Instrumentation Hooks NOT added. Max hooks "
                                "for stage. Stage: %s Module: %s",
                                stage.c_str(), it->module.c_str());
    }
  }
}

const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_header_parser_hooks = {
        appd_hook_header_parser1, appd_hook_header_parser2, appd_hook_header_parser3,
        appd_hook_header_parser4, appd_hook_header_parser5};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_header_parser_indexes = {
        HookContainer::APPD_ENDPOINT_HEADER_PARSER1, HookContainer::APPD_ENDPOINT_HEADER_PARSER2,
        HookContainer::APPD_ENDPOINT_HEADER_PARSER3, HookContainer::APPD_ENDPOINT_HEADER_PARSER4,
        HookContainer::APPD_ENDPOINT_HEADER_PARSER5};
const std::vector<ApacheHooksForStage::quickHandlerHooks>
    ApacheHooksForStage::appd_quick_handler_hooks = {
        appd_hook_quick_handler1, appd_hook_quick_handler2, appd_hook_quick_handler3,
        appd_hook_quick_handler4, appd_hook_quick_handler5};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_quick_handler_indexes = {
        HookContainer::APPD_ENDPOINT_QUICK_HANDLER1, HookContainer::APPD_ENDPOINT_QUICK_HANDLER2,
        HookContainer::APPD_ENDPOINT_QUICK_HANDLER3, HookContainer::APPD_ENDPOINT_QUICK_HANDLER4,
        HookContainer::APPD_ENDPOINT_QUICK_HANDLER5};
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_access_checker_hooks = {
        appd_hook_access_checker1, appd_hook_access_checker2, appd_hook_access_checker3,
        appd_hook_access_checker4, appd_hook_access_checker5};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_access_checker_indexes = {
        HookContainer::APPD_ENDPOINT_ACCESS_CHECKER1, HookContainer::APPD_ENDPOINT_ACCESS_CHECKER2,
        HookContainer::APPD_ENDPOINT_ACCESS_CHECKER3, HookContainer::APPD_ENDPOINT_ACCESS_CHECKER4,
        HookContainer::APPD_ENDPOINT_ACCESS_CHECKER5};
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_check_user_id_hooks = {
        appd_hook_check_user_id1, appd_hook_check_user_id2, appd_hook_check_user_id3,
        appd_hook_check_user_id4, appd_hook_check_user_id5};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_check_user_id_indexes = {
        HookContainer::APPD_ENDPOINT_CHECK_USER_ID1, HookContainer::APPD_ENDPOINT_CHECK_USER_ID2,
        HookContainer::APPD_ENDPOINT_CHECK_USER_ID3, HookContainer::APPD_ENDPOINT_CHECK_USER_ID4,
        HookContainer::APPD_ENDPOINT_CHECK_USER_ID5};
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_auth_checker_hooks = {
        appd_hook_auth_checker1, appd_hook_auth_checker2, appd_hook_auth_checker3,
        appd_hook_auth_checker4, appd_hook_auth_checker5};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_auth_checker_indexes = {
        HookContainer::APPD_ENDPOINT_AUTH_CHECKER1, HookContainer::APPD_ENDPOINT_AUTH_CHECKER2,
        HookContainer::APPD_ENDPOINT_AUTH_CHECKER3, HookContainer::APPD_ENDPOINT_AUTH_CHECKER4,
        HookContainer::APPD_ENDPOINT_AUTH_CHECKER5};
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_type_checker_hooks = {
        appd_hook_type_checker1, appd_hook_type_checker2, appd_hook_type_checker3,
        appd_hook_type_checker4, appd_hook_type_checker5};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_type_checker_indexes = {
        HookContainer::APPD_ENDPOINT_TYPE_CHECKER1, HookContainer::APPD_ENDPOINT_TYPE_CHECKER2,
        HookContainer::APPD_ENDPOINT_TYPE_CHECKER3, HookContainer::APPD_ENDPOINT_TYPE_CHECKER4,
...
const std::vector<ApacheHooksForStage::processRequestHooks> ApacheHooksForStage::appd_fixups_hooks =
    {appd_hook_fixups1, appd_hook_fixups2, appd_hook_fixups3, appd_hook_fixups4, appd_hook_fixups5};
const std::vector<HookContainer::appd_endpoint_indexes> ApacheHooksForStage::appd_fixups_indexes = {
    HookContainer::APPD_ENDPOINT_FIXUPS1, HookContainer::APPD_ENDPOINT_FIXUPS2,
    HookContainer::APPD_ENDPOINT_FIXUPS3, HookContainer::APPD_ENDPOINT_FIXUPS4,
...
const std::vector<ApacheHooksForStage::filterHooks> ApacheHooksForStage::appd_insert_filter_hooks =
    {appd_hook_insert_filter1, appd_hook_insert_filter2, appd_hook_insert_filter3,
     appd_hook_insert_filter4, appd_hook_insert_filter5};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_insert_filter_indexes = {
        HookContainer::APPD_ENDPOINT_INSERT_FILTER1, HookContainer::APPD_ENDPOINT_INSERT_FILTER2,
        HookContainer::APPD_ENDPOINT_INSERT_FILTER3, HookContainer::APPD_ENDPOINT_INSERT_FILTER4,
        HookContainer::APPD_ENDPOINT_INSERT_FILTER5};
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_handler_hooks = {
        appd_hook_handler1,  appd_hook_handler2,  appd_hook_handler3,  appd_hook_handler4,
        appd_hook_handler5,  appd_hook_handler6,  appd_hook_handler7,  appd_hook_handler8,
        appd_hook_handler9,  appd_hook_handler10, appd_hook_handler11, appd_hook_handler12,
        appd_hook_handler13, appd_hook_handler14, appd_hook_handler15, appd_hook_handler16,
        appd_hook_handler17, appd_hook_handler18, appd_hook_handler19, appd_hook_handler20};
const std::vector<HookContainer::appd_endpoint_indexes> ApacheHooksForStage::appd_handler_indexes =
    {HookContainer::APPD_ENDPOINT_HANDLER1,  HookContainer::APPD_ENDPOINT_HANDLER2,
     HookContainer::APPD_ENDPOINT_HANDLER3,  HookContainer::APPD_ENDPOINT_HANDLER4,
     HookContainer::APPD_ENDPOINT_HANDLER5,  HookContainer::APPD_ENDPOINT_HANDLER6,
     HookContainer::APPD_ENDPOINT_HANDLER7,  HookContainer::APPD_ENDPOINT_HANDLER8,
     HookContainer::APPD_ENDPOINT_HANDLER9,  HookContainer::APPD_ENDPOINT_HANDLER10,
     HookContainer::APPD_ENDPOINT_HANDLER11, HookContainer::APPD_ENDPOINT_HANDLER12,
     HookContainer::APPD_ENDPOINT_HANDLER13, HookContainer::APPD_ENDPOINT_HANDLER14,
     HookContainer::APPD_ENDPOINT_HANDLER15, HookContainer::APPD_ENDPOINT_HANDLER16,
     HookContainer::APPD_ENDPOINT_HANDLER17, HookContainer::APPD_ENDPOINT_HANDLER18,
     HookContainer::APPD_ENDPOINT_HANDLER19, HookContainer::APPD_ENDPOINT_HANDLER20};
const std::vector<ApacheHooksForStage::processRequestHooks>
    ApacheHooksForStage::appd_log_transaction_hooks = {
        appd_hook_log_transaction1, appd_hook_log_transaction2, appd_hook_log_transaction3,
        appd_hook_log_transaction4, appd_hook_log_transaction5};
const std::vector<HookContainer::appd_endpoint_indexes>
    ApacheHooksForStage::appd_log_transaction_indexes = {
