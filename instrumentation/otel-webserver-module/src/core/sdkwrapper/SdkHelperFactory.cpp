...
// NOTE : all the validation checks related to otel inputs are done in the
// helper factory. So, these constants are not required outside of this
// execution unit
namespace
{
...
SdkHelperFactory::SdkHelperFactory(std::shared_ptr<TenantConfig> config, const AgentLogger &logger)
    : mLogger(logger)
{
  // Create exporter, processor and provider.

  // TODO:: Use constant expressions
  LOG4CXX_INFO(mLogger, "ServiceNamespace: " << config->getServiceNamespace() << " ServiceName: "
                                             << config->getServiceName() << " ServiceInstanceId: "
  ...
  // NOTE : resource attribute values are nostd::variant and so we need to
  // explicitely set it to std::string
  std::string libraryVersion = MODULE_VERSION;

  // NOTE : InstrumentationLibrary code is incomplete for the otlp exporter in
  // sdk. So, we need to pass libraryName and libraryVersion as resource
  // attributes. Ref :
  // https://github.com/open-telemetry/opentelemetry-cpp/blob/main/exporters/otlp/src/otlp_recordable.cc
  ...
  LOG4CXX_INFO(mLogger, "Tracer created with LibraryName: " << config->getOtelLibraryName()
                                                            << " and LibraryVersion "
                                                            << libraryVersion);

  // Adding trace propagator
  using MapHttpTraceCtx = opentelemetry::trace::propagation::HttpTraceContext;
  mPropagators.push_back(std::unique_ptr<MapHttpTraceCtx>(new MapHttpTraceCtx()));

  // Adding Baggage Propagator
  using BaggagePropagator = opentelemetry::baggage::propagation::BaggagePropagator;
  mPropagators.push_back(std::unique_ptr<BaggagePropagator>(new BaggagePropagator()));
}
...
OtelSpanExporter SdkHelperFactory::GetExporter(std::shared_ptr<TenantConfig> config)
{
  ...
  else
  {
    if (type != OTLP_EXPORTER_TYPE)
    {
      ...
      LOG4CXX_WARN(mLogger, "Received unknown exporter type: "
                                << type << ". Will create default(otlp) exporter");
  ...
  LOG4CXX_INFO(mLogger, "Exporter created with ExporterType: " << type);
  return std::move(exporter);
}

OtelSpanProcessor SdkHelperFactory::GetSpanProcessor(std::shared_ptr<TenantConfig> config,
                                                     OtelSpanExporter exporter)
{
  ...
  {
    processor.reset(new opentelemetry::sdk::trace::SimpleSpanProcessor(std::move(exporter)));
  }
  else
  {
    if (type != BATCH_PROCESSOR)
    {
      ...
      LOG4CXX_WARN(mLogger, "Received unknown processor type: "
                                << type << ". Will create default(batch) processor");
  ...
  LOG4CXX_INFO(mLogger, "Processor created with ProcessorType: " << type);
  return processor;
}

OtelSampler SdkHelperFactory::GetSampler(std::shared_ptr<TenantConfig> config)
{
  ...
  else if (type == PARENT_BASED_SAMPLER)
  {  // TODO
    ;
  }
  else
  {
    if (type != ALWAYS_ON_SAMPLER)
    {
      ...
      LOG4CXX_WARN(mLogger, "Received unknown sampler type: "
                                << type << ". Will create default(always_on) sampler");
