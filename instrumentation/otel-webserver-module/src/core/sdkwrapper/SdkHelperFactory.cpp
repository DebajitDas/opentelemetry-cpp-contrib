...
SdkHelperFactory::SdkHelperFactory(std::shared_ptr<TenantConfig> config,
  ...
  mTracer =
      mTracerProvider->GetTracer(config->getOtelLibraryName(), libraryVersion);
  LOG4CXX_INFO(mLogger, "Tracer created with LibraryName: "
                            << config->getOtelLibraryName()
  ...
  using BaggagePropagator =
      opentelemetry::baggage::propagation::BaggagePropagator;
  mPropagators.push_back(
      std::unique_ptr<BaggagePropagator>(new BaggagePropagator()));
}

OtelTracer SdkHelperFactory::GetTracer() { return mTracer; }

OtelPropagators &SdkHelperFactory::GetPropagators() { return mPropagators; }

OtelSpanExporter
SdkHelperFactory::GetExporter(std::shared_ptr<TenantConfig> config) {
  ...
  if (type == OSSTREAM_EXPORTER_TYPE) {
    ...
    if (type != OTLP_EXPORTER_TYPE) {
      ...
      LOG4CXX_WARN(mLogger, "Received unknown exporter type: "
                                << type
                                << ". Will create default(otlp) exporter");
...
OtelSpanProcessor
SdkHelperFactory::GetSpanProcessor(std::shared_ptr<TenantConfig> config,
                                   OtelSpanExporter exporter) {
  auto processor = OtelSpanProcessor{};
  auto type = config->getOtelProcessorType();
  if (type == SIMPLE_PROCESSOR) {
    processor.reset(new opentelemetry::sdk::trace::SimpleSpanProcessor(
        std::move(exporter)));
  } else {
    if (type != BATCH_PROCESSOR) {
      ...
      LOG4CXX_WARN(mLogger, "Received unknown processor type: "
                                << type
                                << ". Will create default(batch) processor");
      type = BATCH_PROCESSOR;
    }
    opentelemetry::sdk::trace::BatchSpanProcessorOptions options;
    processor.reset(new opentelemetry::sdk::trace::BatchSpanProcessor(
  ...
  if (type == ALWAYS_OFF_SAMPLER) {
    ...
    if (type != ALWAYS_ON_SAMPLER) {
      ...
      LOG4CXX_WARN(mLogger, "Received unknown sampler type: "
                                << type
                                << ". Will create default(always_on) sampler");
