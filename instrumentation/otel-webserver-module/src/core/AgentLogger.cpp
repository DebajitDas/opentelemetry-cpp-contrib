...
bool initLogging(const boost::filesystem::path &configFilePath) {
  if (!configFilePath.is_absolute() ||
      !boost::filesystem::exists(configFilePath)) {
    logStartupError(boost::format("Log config file path invalid: %1%") %
...
AgentLogger getLogger(const std::string &name) {
  ...
  if (!loggerPtr) {
    logStartupError(
        boost::format("Cannot initialize log4cxx - null logger: %1%") % name);
