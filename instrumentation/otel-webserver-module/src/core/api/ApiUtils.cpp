...
/*APPD_SDK_STATUS_CODE ApiUtils::ReadFromEnvinronment(TenantConfig&
tenantConfig)
{
...
ApiUtils::ReadSettingsFromReader(IEnvReader &reader, TenantConfig &tenantConfig,
  ...
  /*status = reader.ReadOptionalFromReader(
      reader, std::string(APPD_SDK_ENV_EXPORT_BATCH_SIZE),
  setOtelExportTimeoutMillis); if(APPD_ISFAIL(status)) return status;*/
        ...
        IEnvReader& reader, const std::string& varName, unsigned short& result)
{
    ...
    {
        LOG4CXX_ERROR(  ApiUtils::apiLogger,
                        boost::format("Environment variable %1% specified in
wrong format: failed to cast %2% to %3% type") % varName.c_str() % value %
e.target_type().name());
        ...
        const std::string& varName, std::string& result)
{
    ...
    if(!pValue)
    {
        LOG4CXX_ERROR(ApiUtils::apiLogger, boost::format("Environment variable
%1% must be specified") % varName.c_str()); return
APPD_STATUS(unspecified_environment_variable);
    ...
    {
        LOG4CXX_ERROR(ApiUtils::apiLogger, boost::format("Environment variable
%1% must be non-empty") % varName.c_str()); return
APPD_STATUS(unspecified_environment_variable);
    }
        ...
        const std::string& varName, std::string& result)
{
    ...
    if(!varValue)
    {
        LOG4CXX_TRACE(ApiUtils::apiLogger, boost::format("Environment variable
%1% is not specified") % varName.c_str()); return APPD_SUCCESS;
    }
    ...
    {
        LOG4CXX_TRACE(ApiUtils::apiLogger, boost::format("Environment variable
%1% is non-empty") % varName.c_str()); return APPD_SUCCESS;
    }
